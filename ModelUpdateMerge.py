...
def merge_new_model(model, model_index,  config, num_models, evaluation, new_model_samples = None):
    ...
    if algorithm == 'CPR' or algorithm == 'CPR_newModels':                                          
        model, evaluation = add_new_model_CPR(model, model_index, config, num_models, evaluation, new_model_samples = None)
    elif algorithm == 'IOM':                                                     
        model, evaluation = add_new_model_IOM(model, config, evaluation, new_model_samples = None)
    elif algorithm == 'ExStream':                                                     
        model, evaluation = add_new_model_ExStream(model, config, evaluation, new_model_samples = None)
    elif algorithm == 'Random':                                            
        model, evaluation = add_new_model_Random(model, config, evaluation, new_model_samples = None)
    elif algorithm == 'Temporal':                      
        model, evaluation = add_new_model_Temporal(model, config, evaluation, new_model_samples = None)
    else: 
...
def add_new_model_CPR(model, model_index, config, num_models, evaluation, new_model_samples = None): 
    ...
    if new_model_samples is not None:
        ...
        evaluation.eval_noise_discarded_merged_process(new_model_samples, final_samples)
    else:
        evaluation.eval_new_model(model, init_samples, model.samples)
    return model, evaluation
...
def add_new_model_IOM(model, config, evaluation, new_model_samples = None):
    ...
    while len(final_samples) > memory_budget:
        if ind % NF_CLEAN == 0: #OBTIENES LOS SCORES DE TODOS LOS CLUSTER DE LA CLASE (LABEL) Y TIRAS EL QUE MENOS SCORE TENGA
...
def add_new_model_ExStream(model, config, evaluation, new_model_samples = None):
    ...
    if new_model_samples is not None:
        ...
        evaluation.eval_new_model(model, init_samples, init_samples)
    return model, evaluation
...
#Global Update of the distance matrix
def add_CPRV0(model, idx_model,  query, memory_budget, evaluation): 
    """
    Data selection process for model adapting
    Parameters:
    -----------
    ...
    if query_diversity > model.diversity: 
        ...
        if len(samples) > memory_budget:                
            #Add query dist to centroids models
            new_queryDist = query.dismat
            inter_cluster_dist = model.cluster_distances  
            ...
            for ind in range(len(features)):           
                ...
                H_model = get_Hmodel(idx_model, features_aux, samples_aux, int_clusterD_aux) 
                H.append(np.sum(H_model))
            H = np.array(H)
        ...
        #If the memory budget is not exceed
            ...
            model.cluster_distances = np.append(model.cluster_distances, query.dismat, axis = 0)
            model, keep = evaluation.eval_query_selected(model, query)

    evaluation.eval_noise_discarded(query, keep)
         
    return model, evaluation, keep

def add_new_model_DunnIndex(app_models, model, ind_model, query, model_size, qid, n_newModel, acc):
    samples = model.samples 
    ...
    if len(features) > model_size:
        ...
        # print('models to update', len(DistancesTOupdate))

        min_Distances = getminDist(app_models, query)
        # print('shape min dist', np.shape(min_Distances))
        model.cluster_distances = np.append(model.cluster_distances,[min_Distances], axis = 0)
        # print('shape cluster distances', np.shape(model.cluster_distances))
